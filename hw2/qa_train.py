...
Original file is located at
    https://colab.research.google.com/drive/14MxneUAetUf8KnbXRgJ7A5ti01yYpK-d
"""
...
#     steps = len(data)
#     x = range(0, steps*2000, 2000) 
#     plt.figure(figsize=(6, 4))
#     plt.plot(x, data, c=f'tab:{color}')
...
def parse_args():
    ...
    parser.add_argument(
        "--with_pretrained_weight",
        type=bool,
        default=True,
        help="whether to load pretrained weight"
    )
    ...
    parser.add_argument("--seed", type=int, default=406, help="A seed for reproducible training.")
    parser.add_argument(
        "--doc_stride",
        type=int,
        default=128,
...
def main():
    ...
    if args.model_name_or_path:
        if not args.with_pretrained_weight:
            config.__dict__["hidden_size"] = 256
            config.__dict__["num_attention_heads"] = 2
    ...
    def post_processing_function(examples, features, predictions, stage="eval"):
        ...
        # print(references[0], type(references[0]["answers"]))
            ...
            if step % 2600 == 0:
                ...
                for _step, _batch in enumerate(eval_dataloader):
                    with torch.no_grad():
                        ...
                        # print(_outputs)
                        start_logits = _outputs.start_logits
                        end_logits = _outputs.end_logits
                ...
                print(f"step: {step}, eval_metric: {eval_metric}, loss: {train_loss}")
                record["loss"].append(train_loss)
                record["EM"].append(eval_metric["exact_match"])
                print(type(train_loss),  type(eval_metric["exact_match"]))
    ...
    # with open('./drive/MyDrive/ADL/hw2_qa/record.json', 'r') as f:
    #     record = json.load(f)
    # plot(record["loss"], "Training loss", 'red')
    # plot(record["EM"], "Exact Match", 'cyan')
